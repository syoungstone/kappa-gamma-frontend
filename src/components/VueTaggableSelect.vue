          ...
          >
            ...
            >
              <svg
                ...
                :class="searchText ? 'opacity-100' : 'opacity-25'"
              ...
              >
                ...
                />
                <path
                  d="M38.5 25H27V14a1 1 0 1 0-2 0v11H13.5a1 1 0 1 0 0 2H25v12a1 1 0 1 0 2 0V27h11.5a1 1 0 1 0 0-2z"
                />
              </svg>
            </span>
            <span
              v-if="!taggable && forceIcons"
              @click.prevent="searchText = searchText !== null ? null : ''"
              :class="[classes.icons]"
              class="cursor-pointer p-1 text-left flex text-grey-darkest"
            >
              <svg
...
<script>
import pointerScroll from "../helpers/pointerScroll";
export default {
  ...
  props: {
    ...
    createOptionKey: {
      ...
      default() {
        return (
          Math.max(...this.mutableOptions.map((opt) => opt[this.optionKey])) + 1
        );
      },
    },
    findExistingOption: {
      ...
      default(option, givenOptions) {
        if (this.optionValue) {
          return givenOptions.find(
            (opt) => opt[this.optionKey] === this.getOptionValue(option)
          );
        }

        if (this.optionLabel) {
          return givenOptions.find(
            (opt) => opt[this.optionLabel] === option[this.optionLabel]
          );
        }
  ...
  watch: {
    ...
    options(curr) {
      this.mututableOptions = curr.slice(0);
    },
  },
  methods: {
    popSelectedOption() {
          ...
          .filter(
            (option) =>
              this.selectedOptions.findIndex(
                (selected) =>
                  selected[this.optionKey] === option[this.optionKey]
              ) < 0
          )
          .filter((option) => {
            return (
